...
var MAGIC_V2 = '  V2';
var FRAME_TYPE_RESPONSE = 0;
var FRAME_TYPE_ERROR = 1;
var FRAME_TYPE_MESSAGE = 2;
...
function unpackMessage(data) {
  ...
  var timestamp = new Int64(data, 0).toOctetString();
  // BigNumber to represent the timestamp in a workable way.
  timestamp = new BigNumber(timestamp, 16);

  var attempts = data.readInt16BE(8);
  var id = data.slice(10, 26).toString();
  var body = data.slice(26);
...
function identify(data) {
  var validIdentifyKeys = ['client_id', 'deflate', 'deflate_level', 'feature_negotiation', 'hostname', 'heartbeat_interval', 'long_id', 'msg_timeout', 'output_buffer_size', 'output_buffer_timeout', 'sample_rate', 'short_id', 'snappy', 'tls_v1', 'user_agent'];

  // Make sure there are no unexpected keys
  var unexpectedKeys = _.filter(_.keys(data), function (k) {
    return !Array.from(validIdentifyKeys).includes(k);
  });
 ...
 */
function requeue(id) {
  var timeMs = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
  ...
  var parameters = ['REQ', null, id, timeMs];
  return command.apply(undefined, parameters);
}
...
function mpub(topic, data) {
  ...
  var messages = _.map(data, function (message) {
    var buffer = new Buffer(4 + byteLength(message));
  ...
  var numMessagesBuffer = new Buffer(4);
 ...
 * @return {Array}
 */
function dpub(topic, data) {
  var timeMs = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
...
function validTopicName(topic) {
  return topic && topic.length > 0 && topic.length < 65 && topic.match(/^[\w._-]+(?:#ephemeral)?$/) != null;
}
...
function validChannelName(channel) {
  return channel && channel.length > 0 && channel.length < 65 && channel.match(/^[\w._-]+(?:#ephemeral)?$/) != null;
}

module.exports = {
  FRAME_TYPE_ERROR: FRAME_TYPE_ERROR,
  FRAME_TYPE_MESSAGE: FRAME_TYPE_MESSAGE,
  FRAME_TYPE_RESPONSE: FRAME_TYPE_RESPONSE,
  MAGIC_V2: MAGIC_V2,
  auth: auth,
  close: close,
  dpub: dpub,
